<template>
  <div>
    ...
    <div class="AddWrap">
      <form>
        <table class="tbAdd">
          ...
          <tr>
            <th>사진</th>
            <td><input type="text" v-model="photo" ref="subject" /></td>
          </tr>
          <tr>
            <th>내용</th>
    ...
    <div class="bntWrap">
      <a href="javascript:;" @click="fnList" class="btn" >취소</a>
      <a href="javascript:;" @click="fnAddProc" class="btnAdd btn">임시저장</a>
...
export default{
  ...
  ,methods:{
    ...
    ,fnAddProc(){   //등록 프로세스
      ...
      this.form = {     //backend로 전송될 POST데이터
        board_code:this.board_code,
        subject:this.subject,
        cont:this.cont,
        id:this.id
      }
      this.$axios.post('http://localhost :8090/api/board', this.form)
      .then((res)=> {
        if(res.data.success) {
          alert('등록되었습니다.');
          this.fnList();
